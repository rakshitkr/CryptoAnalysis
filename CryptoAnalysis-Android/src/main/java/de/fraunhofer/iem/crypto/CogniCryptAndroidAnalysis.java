...
public class CogniCryptAndroidAnalysis {
	...
	public Optional<File> getOutputDir() {
	...
	public CogniCryptAndroidAnalysis(File apkFile, File platformsDirectory, File rulesLocation) {
		this(apkFile, platformsDirectory, rulesLocation, null, Collections.EMPTY_LIST);
	}

	/**
	 * @param apkFile The Android platforms directory.
	 * @param platformsDirectory The Android platform which the .apk targets.
	 * @param outputDir The path where output files get saved.
	 * @param rulesLocation The CrySL rules directory.
	 */
	public CogniCryptAndroidAnalysis(File apkFile, File platformsDirectory, File rulesLocation, File outputDir) {
		this(apkFile, platformsDirectory, rulesLocation, outputDir, Collections.EMPTY_LIST);
	}
	 ...
	 */
	public CogniCryptAndroidAnalysis(String apkFile, String platformsDirectory, String rulesLocation,
									 @$Nullable String outputDir, Collection<String> applicationClassFilter) {
	...
	public static void main(String... args) {
		...
		if (args[3] != null) {
			...
			analysis = new CogniCryptAndroidAnalysis(args[0], args[1], args[2], null, Lists.newArrayList());
		}
		try {
			analysis.run();
		} catch (CryptoAnalysisException e) {
			String message = "Analysis failed with error: " + e.getMessage();
	...
	private Collection<AbstractError> runCryptoAnalysis() throws CryptoAnalysisException {
		...
		if (outputDir != null) {
			reporter.addReportListener(new CommandLineReporter(outputDir.getAbsolutePath(), rules));
		}
	...
	protected List<CrySLRule> getRules() throws CryptoAnalysisException {
		...
		// TODO: Support explicit .crySL file as input
		if (rulesLocation.isDirectory()) {
			rules =  CrySLRuleReader.readFromDirectory(rulesLocation);
		} else if (rulesLocation.getName().endsWith(".zip")) {
			rules = CrySLRuleReader.readFromZipFile(rulesLocation);
		} else {
			throw new RuntimeException(
					"Please specify a directory the CrySL rules (" + CrySLModelReader.cryslFileEnding + " Files) are located in.");
		}
